type CompactState = "active" | "waiting" | "idle" | "stalled" | "finished" | "failed" | "unknown";
type DetailedState =
  | "starting" | "running" | "waiting_for_agent" | "waiting_for_tool" | "waiting_for_user"
  | "idle" | "stalled" | "completed" | "failed" | "cancelled" | "disappeared" | "unknown";
type Scope = "active" | "all";
type Sort = "attention" | "directory";
...
const controls = document.querySelector<HTMLFormElement>("form.controls");

let latest: Snapshot | null = null;
let priorStates = new Map<string, DetailedState>();
...
function createChildSummary(counts: Partial<Record<CompactState, number>>): HTMLElement {
...
function notifyImportantChanges(snapshot: Snapshot): void {
...
function setConnection(state: "live" | "reconnecting" | "stale"): void {
...
function isChildCounts(value: unknown): value is Partial<Record<CompactState, number>> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return false;
  return Object.entries(value).every(([state, count]) => compactStates.has(state) && typeof count === "number" && count >= 0);
}

function isWarning(value: unknown): value is Warning | null {
...
function optionalNumber(value: unknown): boolean { return value === null || typeof value === "number"; }
...
function requiredSelect(id: string): HTMLSelectElement {
  const node = document.getElementById(id);
  if (!(node instanceof HTMLSelectElement)) throw new Error(`Missing required select: ${id}`);
  return node;
}
...
function requiredButton(id: string): HTMLButtonElement {
  const node = document.getElementById(id);
  if (!(node instanceof HTMLButtonElement)) throw new Error(`Missing required button: ${id}`);
  return node;
}

function createEmpty(scope: Scope): HTMLElement {
...
function selectedScope(): Scope { return scopeControl.value === "all" ? "all" : "active"; }
function selectedSort(): Sort { return sortControl.value === "directory" ? "directory" : "attention"; }

function compareCards(left: Card, right: Card, sort: Sort): number {
  const directory = left.startup_directory.localeCompare(right.startup_directory, undefined, { sensitivity: "base" });
  if (sort === "directory") return directory || left.session_id.localeCompare(right.session_id);
  return attentionRank(left.compact_state) - attentionRank(right.compact_state)
    || directory
    || left.session_id.localeCompare(right.session_id);
}

function attentionRank(state: CompactState): number {
...
function statePresentation(state: DetailedState): readonly [string, string] {
  const presentations: Record<DetailedState, readonly [string, string]> = {
...
function safePullRequestUrl(raw: string | null): string | null {
...
const compactStateOrder: readonly CompactState[] = ["active", "waiting", "idle", "stalled", "finished", "failed", "unknown"];
const compactStates = new Set<unknown>(compactStateOrder);
const detailedStates = new Set<unknown>([
