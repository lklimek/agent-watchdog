...
| FR-MCP-013 | T-MCP-019 | Child registration resolves its parent by ID and binds the caller's transport under a rollback guard (`resolve_child_parent`, `ScopeGuard`); `a_spawned_agent_registers_itself_as_a_child_on_its_own_transport`, `a_coordinator_registered_child_binds_its_own_transport_by_re_registering`, `a_bound_transport_cannot_register_a_child_into_another_tree`, `a_rejected_child_registration_leaves_its_transport_unbound` | Automated |
