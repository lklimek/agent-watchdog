...
Status: verified; live-runtime follow-up complete
...
|---|---|---|
| One coordinator session ID silently gates out jobs launched by distinct wrapper sessions. | Not present. | Companion discovery iterates every bounded summary job independently. A unique shared Claude transcript alias reuses the canonical member/team tree; otherwise the exact persisted wrapper remains the parent. `companion_discovery_keeps_jobs_from_distinct_wrapper_sessions` and `companion_wrapper_reuses_unique_claude_team_alias` cover both cases. Explicit MCP delegation can still enrich automatic discovery. |
...
| Persistent failures must not emit unbounded unsanitized diagnostics. | Implemented. | Native inputs are bounded/typed, errors exposed by parsers and reconciliation are content-free, structured `tracing` uses stable event names, durable health records are bounded, and retained diagnostics omit transcript/job-log bodies. Codex correlation decisions use a bounded cache and emit only on the first or a changed outcome rather than on every inotify reconciliation. |
...
- Automatic Companion hierarchy follows a wrapper session's unique Claude
  transcript alias when one exists. It never infers the coordinator from
  `workspaceRoot`, newest-job ordering, or a non-unique cwd; without an exact
  alias it retains the wrapper parent. Agents can supply additional exact
  orchestration knowledge through MCP.
