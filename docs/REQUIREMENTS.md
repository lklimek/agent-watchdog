...
| FR-DIS-010 | Runtime lifecycle records with a supported semantic state must be reduced as typed state, not generic activity. | Codex `task_started` becomes running; `task_complete` means waiting for user on a main session and completed on a child; unknown supported activity remains progress. |
...
| FR-EVD-012 | Worktree prefixes are capability allowlists, not recursive watch requests. Runtime roots, exact uniquely-owned active worktrees, and MCP-registered paths receive bounded inotify coverage, with runtime evidence taking priority. Shared active worktrees do not receive automatic filesystem watches because their changes cannot identify one agent. | A worktree prefix containing more directories than the global target cap cannot starve a small Codex/Claude/Companion runtime root; a uniquely-owned active child worktree remains observable; two active children sharing a worktree do not allocate an automatic target. |
| FR-EVD-013 | Filesystem invalidation is routed to the affected adapter or worktree owner rather than scheduling every adapter. | A Codex runtime event requests Codex reconciliation only; a worktree event requests attribution only; overflow and periodic recovery may still request all adapters. |
...
| FR-STA-007 | Cheap corroborating checks run immediately before an alert. | Alert evidence includes last trusted transition, signal times, PID, process-tree CPU deltas, active-operation summary, conflicts, correlation basis, an explicit outcome-uncertainty flag, and suggested checks. Runtime disappearance requires checking the exact target branch/worktree for newer commits or changes before treating the outcome as failure. |
...
| FR-MCP-003 | Agents can register sessions/delegations, report progress/waiting/completion, manage deadlines, list their tree, read events, and read health. | Tool schemas use runtime-neutral IDs. Every object-shaped response advertises an output schema derived from the type it serializes and returns that JSON as both structured content and backward-compatible text; the array-shaped `list_sessions` response, which MCP structured content cannot carry, remains text. |
...
| FR-MCP-008 | Concurrent authenticated MCP transports are bounded, and an abandoned transport is reclaimed after a finite idle period. Both bounds are operator-configurable in `watchdog.toml` and applied at startup. | Live transports never exceed the configured cap under concurrent admission. An idle transport past the configured TTL is closed, releasing both its admission slot and its application scope. A zero cap or zero idle TTL is rejected when the configuration loads. |
| FR-MCP-009 | Admission at the cap evicts the least-recently-active transport instead of refusing the new connection. | The longest-idle transport is closed through the same release path as idle expiry, more recently active transports keep their scope, occupancy stays at the cap, and the new transport is admitted. Occupancy, capacity, and cumulative evictions are readable through `get_watchdog_health`. |
| FR-MCP-010 | Main registration resolves a uniquely persisted runtime-native discovery alias onto its canonical session, and the authorization scope follows that resolved identity rather than the asserted one. A session stored under the exact runtime-native identity takes precedence over any alias, so a first-party self-assertion is never redirected by a third-party correlation. | Registering an aliased native ID binds the already-discovered canonical main without creating a parallel tree, and a child registered against the returned identity joins that canonical tree. A native ID that already has its own stored session registers against that session regardless of alias evidence. Ambiguous aliases fail closed. |
| FR-MCP-012 | One runtime-native alias key retains one current canonical target, and an alias that can no longer adopt a live caller never redirects one. | A newer correlation supersedes the stored guess instead of accumulating a conflicting row, and an older observation does not demote current evidence. Registration whose alias target is absent, is not a root main, or already established its outcome discards that alias and completes under the caller's own identity, leaving the mistargeted tree unchanged. |
| FR-MCP-013 | A child registration binds the caller's own transport to the tree of the parent session it names. | An agent in a separate process registers itself as a child of a parent `session_id` obtained out of band, without that connection having registered a main first, and can then use its own transport. A transport bound to another tree is still refused, and a rejected child registration grants no scope. |
| FR-MCP-011 | The endpoint bounds how long it waits for one request body, answers a timed-out read explicitly, and records it. The bound is operator-configurable in `watchdog.toml` and applied at startup. | A client that stalls mid-body receives a prompt `408` on the configured bound instead of an open-ended wait, and the server logs the timeout with the configured bound. Long-lived server-push streams are unaffected. A zero bound is rejected when the configuration loads. |
