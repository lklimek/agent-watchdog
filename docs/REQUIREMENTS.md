...
| FR-MCP-010 | Main registration resolves a uniquely persisted runtime-native discovery alias onto its canonical session, and the authorization scope follows that resolved identity rather than the asserted one. | Registering an aliased native ID binds the already-discovered canonical main without creating a parallel tree, and a child registered against the returned identity joins that canonical tree. Ambiguous aliases fail closed. |
