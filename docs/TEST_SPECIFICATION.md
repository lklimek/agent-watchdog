...
| T-MCP-019 | A transport that never registered a main registers a child against another transport's main session, re-registers an already-registered child, targets a parent in a foreign tree, and asserts an identity another main already owns. | Both self-registrations succeed and bind the child's own transport while the coordinator keeps its scope, the foreign-tree target is rejected as cross-tree without persisting a session, and the conflicting registration is rejected and leaves its transport unbound. | FR-MCP-013 |
