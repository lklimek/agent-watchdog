...
| T-DIS-013 | Codex rollout emits `event_msg.payload.type=task_started`, then `task_complete`. | A child transitions running then completed; a main transitions running then waiting for user; transcript content is neither retained nor logged. | FR-DIS-010, FR-SEC-001 |
...
| T-STA-018 | A Companion job remains `running` with a stale activity timestamp, its verified runtime disappears, and a real target-branch commit is newer than that timestamp. Separately, a child runs normally and then completes; and a child raises a source conflict and later reports a terminal state. | The disappearance alert remains immediate, but its diagnostics mark the terminal outcome uncertain and require an exact target-branch/worktree cross-check before failure is inferred. A plainly running child and a normally completed child are both reported certain. A terminal state reached while a source conflict is still unresolved remains uncertain. | FR-STA-006, FR-STA-007 |
...
| T-MCP-010 | A real rmcp client lists every object-shaped tool and calls representative successful responses. | Each such tool advertises an object output schema. Called responses provide schema-valid structured content and equivalent JSON text for legacy clients. | FR-MCP-003 |
| T-MCP-011 | Configuration omits `[mcp]`, then sets explicit bounds, then sets a zero cap. | Omission yields the shipped defaults, explicit bounds are applied, and a zero cap or zero idle TTL is rejected when the configuration loads. | FR-MCP-008 |
| T-MCP-012 | Transports fill the configured cap with differing last-activity times, then one more connection arrives. | The longest-idle transport is closed and its application scope released, more recently active transports keep theirs, occupancy stays at the cap, the new transport is admitted, and `get_watchdog_health` reports occupancy and the eviction count. | FR-MCP-009 |
| T-MCP-013 | Twice the cap in concurrent create and restore admissions race through a barrier while the admission mutex is held. | Every caller is admitted, no interleaving leaves more live transports than the cap, and the eviction count equals the excess. | FR-MCP-008 |
| T-MCP-014 | A `discovery_aliases` row exists for a runtime-native ID with no `sessions` row, and a hook ingestion asserts that native ID as its parent. | The child registers against the canonical main, no parallel tree is created, and the runtime evidence is retained rather than dropped. | FR-MCP-010 |
| T-MCP-015 | A client sends MCP request headers and then never completes the body, and separately a server-push stream is left open past the same bound. | The stalled request is answered `408` on the configured bound and logged as `mcp.request_body_timeout`; the server-push stream stays open. Omitted configuration yields the shipped default and a zero bound is rejected at load. | FR-MCP-011 |
...
| T-MCP-019 | Fresh transports register children against both main and child parents; an existing child re-registers; invalid identity fails before binding; persistence fails after binding and an exact retry follows a committed observation plus failed relation write; an older completed registration is retried after a newer parent selection; one registration event is reused with a different parent; concurrent same-root registrations commit and roll back in both orders; and a released transport identity is reused while stale guards remain. A bound transport also targets a parent in a foreign tree. | Main and nested child registration select the actual parent and authorize the complete root tree only after persistence commits. Pre-bind and post-bind failures grant no scope, an exact retry repairs a missing relation but cannot supersede a newer relation, mismatched event reuse is rejected, concurrent registrations cannot expose pending scope or undo a committed peer, and stale guards cannot commit or roll back a replacement binding. The foreign-tree target is rejected without persistence. | FR-MCP-013 |
